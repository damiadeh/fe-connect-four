...
describe('GameBoard - Core Functionality', () => {
  ...
  it('should handle column click and drop piece', () => {
    ...
    // Status should be empty (no turn message shown)
    const statusElement = screen.getByRole('heading', { level: 6 })
    expect(statusElement).toHaveTextContent('')
    
    // Should have one red piece placed in the board
    const redPieces = screen.getAllByText('🔴')
    expect(redPieces).toHaveLength(1) // 1 placed piece
    
    // Should now show black pieces in drop indicators
    const blackPieces = screen.getAllByText('⚫')
    expect(blackPieces).toHaveLength(4) // 4 drop pieces for black player
  })
